...
db.autores.find({ nombre_usuario: "juanperez" })

// Por cuenta de Twitter
db.autores.find({ cuenta_twitter: "@juanperez" })
...
db.autores.aggregate([
  { $group: { _id: "$direccion_postal.codigo_postal", count: { $sum: 1 } } }
])
...
```javascript
db.autores.find({ telefonos: "+34123456789" })
...
### 5. 10 últimas noticias publicadas ordenadas por fecha (de más reciente a más antigua). 
 ...
 ```javascript
db.noticias.find().sort({ fecha_publicacion: -1 }).limit(10)
...
db.comentarios.aggregate([
  { $group: { _id: "$noticia_id", count: { $sum: 1 } } }
])

// Por día
db.comentarios.aggregate([
  { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$fecha_comentario" } }, count: { $sum: 1 } } }
])

// Por usuario
db.comentarios.aggregate([
  { $group: { _id: "$autor_id", count: { $sum: 1 } } }
])
...
```javascript
db.autores.find({ nombre_usuario: "Frank_blog" })
...
```javascript
db.autores.countDocuments({ "direccion_postal.codigo_postal": "39005" })
...
```javascript
db.autores.find({ telefonos: "111111111" })
...
```javascript
db.autores.find(
  { "direccion_postal.codigo_postal": { $gte: "39005" } },
  { _id: 0, nombre_usuario: 1, cuenta_twitter: 1 }
)
...
// Consultas por año
db.noticias.aggregate([
  { $project: { year: { $year: "$fecha_publicacion" } } },
  { $group: { _id: "$year", count: { $sum: 1 } } }
])

// Consultas por mes
db.noticias.aggregate([
  { $project: { month: { $month: "$fecha_publicacion" } } },
  { $group: { _id: "$month", count: { $sum: 1 } } }
])

// Consultas por día
db.noticias.aggregate([
  { $project: { day: { $dayOfMonth: "$fecha_publicacion" } } },
